...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = ['name', 'email', 'password', 'phone', 'company_id', 'name_company', 'created_by', 'type', 'otp', 'otp_sent_at', 'is_otp_verified', 'is_active', 'last_logged_in_on', 'deactivated_at'];
    ...
    /**
     * Scope for recently logged in users
     */
    public function scopeRecentlyLoggedIn($query, $days = 7)
    {
        return $query->where('last_logged_in_on', '>=', now()->subDays($days));
    }
    ...
    // public function resolveCurrentCompany()
    // {
    ...
    public function getCompanyDetails()
    {
        ...
        if ($this->hasRole('owner')) {
            return $this->company; // Owner's company details
        } else {
            return $this->company; // Staff's associated company details
        }
    }
    ...
    {
        return $this->hasOne(Subscription_Info::class, 'company_id', 'company_id')
            ->latest()
