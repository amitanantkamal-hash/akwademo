$(document).ready(function () {
    ...
    $(document).on('click', '#move-to-group-confirm', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request
                $.ajax({
                    url: assignToGroup, // Update with actual route
                    type: "POST",
                    data: {
                        _token: csrf_token,
    ...
    $(document).on('click', '#remove-from-group-confirm', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: removeFromGroup, // Update with your route
                    type: "POST",
                    data: {
                        _token: csrf_token,
    ...
    $(document).on('click', '#subscribe-contact', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: bulkSubscribe, // Update with your actual route
                    type: "POST",
                    data: {
                        _token: csrf_token,
                        ids: selectedContacts
    ...
    $(document).on('click', '#unsubscribe-contact', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: bulkUnSubscribe, // Update with actual route
                    type: "POST",
                    data: {
                        _token: csrf_token,
                        ids: selectedContacts,
    ...
    $(document).on('click', '#delete-selected', function () {
        if (isStaff) {
            // Staff users cannot delete
            Swal.fire({
                icon: 'warning',
                title: 'Permission Denied',
                text: 'You do not have proper rights to delete selected contacts.',
                confirmButtonColor: '#3085d6',
