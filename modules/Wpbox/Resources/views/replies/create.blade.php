                            ...
                            <div class="card-body">
                                ...
                                @if ($setup['isBot'] == true)
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <span class="form-info">Optional: max 60 characters allowed. Add a short line to
                                ...
                                @php
                                    ...
                                    if (request()->query('bot_type') != 'catalog') {
                                        $botType = true;
                                    }
                                @endphp
                                ...
                                <div class="card-body pt-0">
                                    <div class="card-body d-flex flex-column align-items-end"
                                        ...
                                        <div id="previewElement" class="card shadow-lg w-100 mb-1"
                                            ...
                                            <div class="card-body pt-0 mt-6" style="background-color: #ffffff;">
                                                <img src="{{ $item->header_image ?? asset('uploads/files/default.png') }}"
                                                    class="card-img-top imageContainer w-100" id="card-img-top"
                                                    style="@if (empty($item->header_image)) display: none; @endif" />

                                                <h6 class="mt-6 mb-2 py-0 fs-11" id="headerTextPreview">
                                                    @isset($item->header)
                                                        {{ $item->header }}
                                                    @endisset
                                                </h6>
                                                ...
                                                <span class="text-muted text-xs" id="footerTextPreview">
                                                    @isset($item->footer)
                                                        {{ $item->footer }}
                                                    @endisset
                                                </span>
                                    ...
                                    <div class="d-flex justify-content-end">
                                        <button type="submit" class="btn btn-info mt-3">
                                            <i class="ki-outline ki-save fs-2 me-1"></i> Save BotReply
                                        </button>
                                    </div>
                                </div>
                            </div>
...
@section('topjs')
    ...
    <script>
        $('#header').on('change', function(e) {
            ...
            if (valueSelected == "TEXT") {
                ...
                document.getElementById("card-img-top").src = '{{ asset('backend/Assets/img/no-image.jpg') }}';
    ...
    // </div>`;
        ...
        var BUTTON_LIST_TEMPLATE = `    
<div class="button_preview border-0 shadow-sm bg-light-info rounded card mb-1" style="text-align: center;">
    <div class="card-body" style="padding:1rem">
        <img class="button_list_icon" style="height: 18px" src="{{ asset('backend/Assets/img/{icon}.png') }}" />
        ...
        @isset($item['button_info'])
            ...
            @if ($item['action_type'] == 3)
                var button_name = '{{ $item['button_info']->button_text }}';
                var option = BUTTON_LIST_TEMPLATE;
                option = option.replace(/{button_list_name}/g, button_name);
                option = option.replace(/{icon}/g, 'list');
                $("#button_list_template").append(option);
            @elseif ($item['action_type'] == 2)
                var button_info = '{{ $item['button_info']->template }}';
                var button_obj = JSON.parse(button_info.replace(/&quot;/g, '"'));
                var button_count = Object.keys(button_obj.templateButtons).length;
                // console.log('>>>>>>>>> button_temp ' + button_count);
                $(".button_preview").remove();
                for (let i = 0; i < button_count; i++) {
                    var button_name = button_obj.templateButtons[i].quickReplyButton.displayText;
                    var option = BUTTON_LIST_TEMPLATE;
                    option = option.replace(/{button_list_name}/g, button_name);
                    option = option.replace(/{icon}/g, 'reply');
                    $("#button_list_template").append(option);
                }
            @endif
        @endisset
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#repliesform').on('submit', function(e) {
                ...
                // Add bot_type if it exists in URL
                if (botType) {
        ...
        `;
            ...
            function showAlert(type, title, message) {
                ...
                $alert.find('.alert-icon-class').addClass('fa-' + (type === 'success' ? 'check-circle' :
                    'exclamation-circle'));
