<script>
    ...
    var initPusher = function() {
        if (typeof Pusher !== 'undefined') {
            ...
            // Pusher
            js.notify("Error: Pusher is not defined. Chat will not load new messages. Please check documentation",
                "danger");
        }
    }
            ...
            if (response.data.status) {
                ...
                if (chatList.contacts.length > 0) {
                    ...
                    if (chatList.activeChat.id == null) {
                        ...
                        const index = chatList.contacts.findIndex(item => item.id === chatList.activeChat
                            .id);
                        if (index !== -1) {
    ...
    window.onload = function() {
        ...
        chatList = new Vue({
            ...
            computed: {
                availableGroups() {
                    // Convert groups object to array of {id, name}
                ...
                filteredFetcherData() {
                    ...
                    return this.fetcherModules[this.selectedFetcher].data.filter(item => item.title
                        .toLowerCase().includes(filterFetcher));
                }
            },
            watch: {
                page(newVal, oldVal) {
            ...
            methods: {
                ...
                async toggleRecording() {
                    if (!this.activeChat?.id) {
                        js.notify("Open a chat first.", "warning");
                        return;
                    }
                    if (!this.isRecording) {
                        // Start
                        try {
                            ...
                            // Pick a WhatsApp-friendly mime if possible
                            let mime = '';
                            if (window.MediaRecorder) {
                                if (MediaRecorder.isTypeSupported('audio/ogg;codecs=opus')) {
                                    mime = 'audio/ogg;codecs=opus';
                                    this.recordingMime = 'audio/ogg';
                                } else if (MediaRecorder.isTypeSupported('audio/webm;codecs=opus')) {
                                    mime = 'audio/webm;codecs=opus';
                                    this.recordingMime = 'audio/webm';
                                } else if (MediaRecorder.isTypeSupported('audio/mp4')) {
                                    mime = 'audio/mp4';
                                    this.recordingMime = 'audio/mp4';
                                }
                            }

                            this.recordedChunks = [];
                            this.mediaRecorder = new MediaRecorder(stream, mime ? {
                                mimeType: mime
                            } : undefined);
                            ...
                            this.mediaRecorder.onstop = async () => {
                                ...
                                const ext = this.recordingMime === 'audio/webm' ? 'webm' :
                                    this.recordingMime === 'audio/mp4' ? 'm4a' :
                                    'ogg';
                                const blob = new Blob(this.recordedChunks, {
                                    type: this.recordingMime || 'audio/ogg'
                                });
                            ...
                            this.mediaRecorder.start(250); // collect in small chunks
                            this.isRecording = true;
                        } catch (e) {
                            console.error(e);
                            js.notify("Microphone blocked/unavailable.", "danger");
                        }
                    } else {
                        // Stop
                        try {
                ...
                async sendAudioMessage(file) {
                    ...
                    try {
                        const {
                            data
                        } = await axios.post(`/api/wpbox/sendaudio/${this.activeChat.id}`, formData, {
                            headers: {
                                'Content-Type': 'multipart/form-data'
                            },
                        });
                        if (!data.status) {
                            js.notify(data.errMsg || "Failed to send audio", "danger");
                        }
                    } catch (err) {
                        console.error(err);
                        js.notify("Failed to send audio", "danger");
                    }
                },
                ...
                filterChatTemplateBySearch() {
                    const searchTerm = this.searchChatTemp.toLowerCase();
                    this.chatTemplates = this.chatTemplatesOrg.filter(template => {
                ...
                filterContactsBySearch() {
                    ...
                    this.contacts = this.all.filter(contact => {
                ...
                formatIt: function(message) {
                    ...
                    // Replace links
                    var replacedText = message.replace(linkRegex,
                        '<a href="$&" class="text-bold" target="_blank">$&</a>');

                    // Bold: *text*
                    replacedText = replacedText.replace(/\*(.*?)\*/g, "<b>$1</b>");

                    // Italic: _text_
                    replacedText = replacedText.replace(/_(.*?)_/g, "<i>$1</i>");
                ...
                translationLanguage(contact) {
                    return contact.language && contact.language != "none" ? contact.language :
                        "{{ __('No translation') }}";
                },
                setLanguage: function(lang, contact) {
                    axios.post('/api/wpbox/setlanguage/' + contact.id, {
                        language: lang
                ...
                assignUser: function(user_id, contact_id) {
                    axios.post('/api/wpbox/assign/' + contact_id, {
                        user_id: user_id
                    }).then(function(response) {
                        if (response.data.status) {
                            chatList.activeChat.user_id = user_id;
                            const indexUpdate = chatList.all.findIndex(item => item.id ==
                                contact_id);
                ...
                getReplyNotification(contact) {
                    var timeSinceLastClientReply = moment.tz(contact.last_client_reply_at, serverTimezone)
                        .add(24, 'hours');
                ...
                sendNote() {
                    ...
                    this.activeNote = "";
                    axios.post('/api/wpbox/sendnote/' + chatList.activeChat.id, {
                        note: note
                    }).then(function(response) {
                        if (response.data.status) {
