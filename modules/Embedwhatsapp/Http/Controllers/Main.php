...
class Main extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!$widget) {
            ...
            $widget['url'] = config('app.url') . '/uploads/default/dotflo/widget/';
            $widget['logo'] = $imageLink; ////config('app.url').$imageLink;
            $widget['chatlink'] = 'https://wa.me/' . $widget['phone_number'];
            if ($widget['widget_type'] == 2) {
                $widget['chatlink'] = 'https://wa.me/' . $widget['phone_number'] . '?text=' . urlencode($widget['input_field_placeholder']);
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        $widget = Whatsappwidget::where('company_id', $companyId)->first();
        ...
        if ($request->hasFile('logo')) {
            $widget->logo = $this->saveImageVersions('uploads/companies/', $request->logo, [['name' => 'large']]);
            $widget->save();
        }
