<template>
	...
	<div class="main-wrapper-header sticky-tops sticky-offset" @click="emit('closeAllModals')">
		...
		<div class="quick-access-area">
			...
			<!-- Modal -->
			<button v-if="data.user.source == 'whatsapp' || data.user.source == 'messenger'" type="button" class="documentory-modal-btn btn"
			...
			<template #content class="pb-0" style="margin-bottom: 0;">
				...
				<div class="modal__scroll p-0">
					<ul class="author-card-list mb-0">
						<li v-for="(contact, index) in data.contacts.filter(contact => contact.source === 'whatsapp')" :key="index" @click="selectContact(contact)">
