...
namespace Modules\BotFlow\Http\Controllers;
...
{
    protected OpenAIAssistantService $aiAssistant;

    public function __construct(OpenAIAssistantService $aiAssistant)
    {
    ...
    {
        $flow = BotFlow::where('company_id', auth()->user()->company->id)
            ->findOrFail($id);
        
        return view('bot-flow::bot-flows.edit', [
            'flow' => $flow,
            'isAiAssistantModuleEnabled' => $this->aiAssistant->enabled(),
            'personalAssistant' => $this->aiAssistant->getAssistantForFlow($flow),
        ]);
    }

    // public function edit($id)
    // {
    //     $flow = BotFlow::where('company_id', auth()->user()->company->id)->findOrFail($id);
    //     $isAiAssistantModuleEnabled = $this->aiAssistant->enabled();

    //     return view('bot-flow::bot-flows.edit', compact('flow', 'isAiAssistantModuleEnabled'));
    // }
    ...
    public function save(Request $request)
    {
        ...
        if ($request->id) {
            $flow = BotFlow::where('company_id', auth()->user()->company->id)->findOrFail($request->id);
            ...
            $message = __('flow_updated_successfully');
        } else {
    ...
    public function delete($id)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => __('flow_deleted_successfully'),
        ]);
    }
}
