<div class="app-navbar flex-lg-grow-1" id="kt_app_header_navbar">
    ...
    @if (!auth()->user()->hasrole(['staff']) && auth()->user()->type == 3)
        @if (\App\Models\Partner::where('user_id', auth()->user()->id)->value('is_active') == 1)
            @include('client.layouts.navbars.navs.items.partner-dashboard')
        @endif
    @endif
    {{-- @if (env('ENABLE_MULTI_ORGANIZATIONS'))
        @include('client.layouts.navbars.navs.items.switch-organization')
    @endif --}}
    {{-- @if (!auth()->user()->hasrole(['staff']))
        @include('client.layouts.navbars.navs.items.agent')
    @endif
    @include('client.layouts.navbars.navs.items.widget')
    @if (!auth()->user()->hasrole(['staff']))
    ...
    @endphp
    <div class="vr mx-2 text-muted" bis_skin_checked="1"></div>
    <div class="app-navbar-item ms-1 ms-md-3" id="kt_header_user_menu_toggle">
        <div class="cursor-pointer symbol symbol-20px d-flex" data-kt-menu-trigger="{default: 'click', lg: 'hover'}"
            data-kt-menu-attach="parent" data-kt-menu-placement="bottom-end">
            <img src="{{ $avatar }}" alt="user" style="border-radius: 50%;" />
            <div class=" ms-3 d-flex align-items-center text-black fs-7">{{ auth()->user()->name }} <i
                    class="ki-outline ki-down fs-4 text-black ms-2"></i></div>
        </div>
        ...
        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg menu-state-color fw-semibold py-4 fs-6 w-350px"
            ...
            @if (env('ENABLE_MULTI_ORGANIZATIONS') == true)
                @if (config('settings.app_code_name', '') == 'wpbox' && auth()->user()->hasRole('owner'))
                    <div class="menu-item px-5">
                        <a href="{{ route('admin.organizations.manage') }}"
                            class="menu-link px-5">{{ __('Switch Organization') }}</a>
                    </div>
                @endif
            @endif
            @if (config('settings.app_code_name', '') == 'wpbox' && auth()->user()->hasRole('owner'))
            ...
            @endif
            <div class="menu-item px-5">
                <a href="https://business.facebook.com/billing_hub/accounts/details/" taget="_blank"
                    class="menu-link px-5">{{ __('Pay Meta Billing') }}</a>
            </div>
            @if (!auth()->user()->hasrole(['staff']))
                <div class="menu-item px-5">
            ...
            <div class="menu-item px-5">
                <div class="d-flex m-4">
                    <div class="flex-1 d-flex justify-content-center">
                        <div>
                            <img src="https://img.icons8.com/color/48/qr-code--v1.png" alt="QR">
                        </div>
                    </div>
                    <div class="flex-grow-1 d-flex flex-stack ms-4">
                        <div class=" fw-semibold">
                            <h4 class="text-gray-900 fs-6 fw-bold">{{ __('Download the mobile app') }}</h4>
                            <div class="fs-9 text-gray-700 ">
                                {{ __('Respond to your customers from your mobile device, and never miss
