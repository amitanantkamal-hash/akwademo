<div class="mb-4">
    <div class="card-body p-0">
        @if (isset($fields[6]['listbuttons']) && !empty($fields[6]['listbuttons']))
            @php $data = $fields[6]['listbuttons']; @endphp

            @if (!empty($interactive_list_id))
                @include('partials.select', [
                    'name' => 'List button templates group',
                    'class' => 'col-md-12',
                    'id' => 'listbtn_template_group_id',
                    'ftype2' => 'yes',
                    'placeholder' => 'Select a group',
                    'value' => '',
                    'dataSelected' => $interactive_list_id ?? '',
                    'data' => $data,
                    'required' => false,
                ])
            @else
