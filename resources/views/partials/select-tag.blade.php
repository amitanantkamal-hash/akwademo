@if ($id === 'groups[]')
    ...
    <input class="form-select form-select-solid d-flex align-items-center" name="groups"
        placeholder="Select a group" id="kt_tagify_users" />
@else
    <div id="form-group-{{ $id }}"
        class=" bg-info form-group w-100 mt-2 mt-sm-2 ms-sm-0 mt-md-2 ms-md-0 mt-lg-2 ms-lg-0 mt-xl-2 ms-xl-0">
        @isset($separator)
        ...
        <div class="d-flex flex-row justify-content-between align-items-center w-100">
            <select @isset($multiple) multiple=" {{ 'multiple' }}"   @endisset
                ...
                class="form-select form-select-solid w-100  @isset($classselect) {{ $classselect }} @endisset"
                name="{{ $id }}" id="{{ $id }}">
                @if (!isset($multiple))
            ...
            @isset($additionalInfo)
                <a class="ms-2" href="{{ $additionalInfo }}"> {!! $additionalInfoLabel !!}</a>
            @endisset
        </div>
        {{-- @if ($errors->has($id))
