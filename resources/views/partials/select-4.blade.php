    <div class="col-md-4">
        <div id="form-group-{{ $id }}"
            class="form-group {{ $errors->has($id) ? ' has-danger' : '' }} mt-2 mt-sm-2 ms-sm-0 mt-md-2 ms-md-0 mt-lg-2 ms-lg-0 mt-xl-2 ms-xl-0 w-100">
            @isset($separator)
            ...
            <div class="d-flex flex-row justify-content-between align-items-center w-100 ">
                <select @isset($multiple) multiple=" {{ 'multiple' }}"   @endisset
                    ...
                    class="form-select form-select-solid @isset($classselect) {{ $classselect }} @endisset"
                    name="{{ $id }}" id="{{ $id }}">
                    @if (!isset($multiple))
                ...
                @isset($additionalInfo)
                    <a class="ms-2 me-0 me-sm-0 me-md-0 me-lg-0 me-xl-0" href="{{ $additionalInfo }}">
                        {!! $additionalInfoLabel !!}</a>
                @endisset
            </div>
            @if ($errors->has($id))
