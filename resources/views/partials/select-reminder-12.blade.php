<div class="{{ isset($fullcol) ? ' col-12' : ' col-md-6' }}  ">
    ...
    <div id="form-group-{{ $id }}" class="form-group {{ $errors->has($id) ? ' has-danger' : '' }} mt-2 mt-sm-2 ms-sm-0 mt-md-2 ms-md-0 mt-lg-2 ms-lg-0 mt-xl-2 ms-xl-0 w-100">
        ...
        <div class="d-flex flex-row justify-content-between align-items-center w-100 ">
            <select  {{--para modal data-dropdown-parent="#kt_modal_create"--}} data-hide-search="true" data-placeholder="{{ __('Select') . ' ' . __($name) }}" data-control="select2" @isset($multiple) multiple="multiple" @endisset
                ...
                @foreach ($data as $key => $item)
                    @php
                        // Verifica si $values está definido y es un array
                        $isSelected = false;
                    ...
                    @endphp
                    @if (is_array(__($item)))
                        <option value="{{ $key }}" @if($isSelected) selected @endif>{{ $item }}</option>
                    @else
                        <option value="{{ $key }}"
                            @if ($isSelected) selected
                            @elseif (old($id) && old($id) . '' == $key . '') selected
                            @elseif (isset($value) && trim(strtoupper($value . '')) == trim(strtoupper($key . ''))) selected
                            @elseif (app('request')->input($id) && strtoupper(app('request')->input($id) . '') == strtoupper($key . '')) selected
                            @elseif (isset($multiple) && isset($multipleselected) && in_array($key, $multipleselected, false)) selected
                            @endif
                        >
                            {{ __($item) }}
                        </option>
