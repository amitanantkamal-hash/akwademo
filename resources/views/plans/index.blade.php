                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('plans.create') }}"
                                    class="btn btn-sm btn-primary">{{ __('Add plan') }}</a>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($plans as $plan)
                                        <tr>
                                            ...
                                            @endif
                                            <td>{{ $plan->period == 1 ? __('Monthly') : ($plan->period == 3 ? __('Lifetime') : __('Anually')) }}
                                            </td>
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                        <a class="dropdown-item"
                                                            href="{{ route('plans.edit', $plan) }}">{{ __('Edit') }}</a>
                                                        @if ($plan->id != 1)
                                                            <form action="{{ route('plans.destroy', $plan) }}"
                                                                ...
                                                                <button type="button" class="dropdown-item"
                                                                    onclick="confirm('{{ __('Are you sure you want to delete this plan?') }}') ? this.parentElement.submit() : ''">
                                                                    {{ __('Delete') }}
                                                                </button>
                                                            </form>
                                                        @endif
