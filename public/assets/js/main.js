...
$(function(){
    $('.icon-password').on('click', function (e) {
        ...
        const iconWidth = 40;
        const iconPosition = $passwordField.outerWidth() - iconWidth; 
        if (e.pageX >= $passwordField.offset().left + iconPosition) {
            const cursorPosition = $passwordField[0].selectionStart;
            const type = $passwordField.attr('type') === 'password' ? 'text' : 'password';
            $passwordField.attr('type', type);
            setTimeout(function () {
                $passwordField[0].setSelectionRange(cursorPosition, cursorPosition);
            }, 0);
        }
    });
})
